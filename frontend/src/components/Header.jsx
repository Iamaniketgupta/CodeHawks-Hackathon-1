  ...
  return (
    <div className="sticky top-0 flex h-14 dark:bg-[#0e1012]  bg-white text-slate-800 items-center justify-between px-8 py-3">
      <div className="font-bold text-xl min-w-fit">
        <Link
          to={"/"}
          className="dark:text-white text-slate-800 hover:text-inherit"
        >
          Code Hawks
        </Link>
      </div>
      {status ? (
        <nav className=" max-w-fit flex items-center justify-center gap-2 pl-4">
          <div>
            {dark ? (
              <CiLight
                size={25}
                className="cursor-pointer  text-yellow-300 shadow-lg font-bold"
              ...
              <MdDarkMode
                size={25}
                className="cursor-pointer  hover:text-blue-700 dark:text-white text-slate-800"
          ...
          <div onClick={logout1}>
          <div
            className=" min-w-fit p-2 px-4 bg-gray-500 font-semibold  hover:text-blue-500 cursor-pointer font-semibold text-sm rounded-3xl"
          >
            Log out
          </div>
          </div>
        ...
        <nav className=" max-w-fit flex items-center justify-center gap-2 pl-4">
          ...
          <Link
            to={"/api/login"}
            className=" min-w-fit p-2 px-4  hover:text-blue-500 cursor-pointer font-semibold text-sm rounded-3xl"
          >
            Log in
          </Link>
          <Link
            to={"/api/signup"}
            className="min-w-fit p-2 px-4  bg-blue-700 cursor-pointer hover:text-inherit text-white font-semibold text-sm rounded-3xl"
          >
            Sign up
          </Link>
        </nav>
        ...
        <div className='sticky top-0 z-50 flex h-14 dark:bg-[#0e1012] bg-white text-slate-800 items-center justify-between px-8 py-3'>
            ...
            <nav className="max-w-fit flex items-center justify-center gap-2 pl-4">
                ...
                {location.pathname !== "/api/dashboard" ? (
                    ...
                    <Link to={"/api/login"} className='min-w-fit p-2 px-4 hover:text-blue-500 cursor-pointer font-semibold text-sm rounded-3xl'>Log Out</Link>
                )}
            </nav>
        </div>
    );
